            ...
            success: function (response) {
                ...
                if (Object.keys(results).length > 0) {
                    $.each(results, function (j, lubes) {
                        var amnt_bbf = lubes.amount;
                        if (lubes.debit_type == 1)
                            amnt_bbf = (lubes.amount);
                        debit = parseFloat(lubes.debit);
                        bbf = parseFloat(lubes.bbf) - parseFloat(amnt_bbf);
                        balance = parseFloat(bbf) - parseFloat(lubes.debit) + parseFloat(lubes.credit);
                        sum_balance += parseFloat(balance);
                        total = parseFloat(debit) + parseFloat(bbf);
                        sum_total += parseFloat(total);
                        total_bbf += parseFloat(bbf);
                        total_credit += parseFloat(lubes.credit);
                        total_debit += parseFloat(debit);
                        trHTML += draw_center(lubes);
                    });
                }
                totals.bbf = total_bbf;
                totals.credit = total_credit;
                totals.debit = total_debit;
                totals.total = sum_total;
                totals.balance = sum_balance;
