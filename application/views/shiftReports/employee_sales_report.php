<div class="pageheader">
    <h2><i class="fa fa-sliders"></i> Employee Sales Summary Report</h2>
    <p class="mb20 text-muted">View and Filter Employee Sales Summary Report for given date range</p>
</div>
                ...
                <div id="export_div" style="display:none">
                    <div class="col-md-12">
                        <form action="<?php echo base_url('Export') ?>" method="POST" target="_blank">
                            <input type="hidden" name="sub_heading" id="sub_heading" value='<?php echo json_encode(array('sub_heading_1' => 'Employee Sales Summary Report', 'sub_heading_2' => '')) ?>'>
...
<script type="text/javascript">
    $("#generate_report").click(function (e) {
        ...
        $.ajax({
            url: '<?php echo base_url("ShiftReports/employee_sales_report") ?>',
            type: 'POST',
            ...
            success: function (response) {
                ...
                $.each(categories, function (i, item) {
                    ...
                    if (Object.keys(data_response).length > 0) {
                        ...
                        trHTML += total_table(item.name, sum_vol, sum_amnt, sum_netamnt, sum_vat_amnt, sum_non_vat_amnt, sum_zero_amnt);
                    }
                });
                trHTML += close_main_table(g_sum_vol, g_sum_amnt, g_sum_netamnt, g_vat_amnt, g_non_vat_amnt, g_zero_amnt);
                new_table += trHTML;
    ...
    function initialize_export(data) {
        ...
        sub_heading['sub_heading_1'] = 'Employee Sales Summary Report for a Period of ' + date_range;
    ...
    function open_main_table() {
        draw_panel = '<table class="table table-condensed mb0"><thead><tr font-size:8px><th width="20%">Description</th><th width="12%">Volume</th><th width="14%">Total Sales</th><th width="14%">Net Sales</th><th width="12%">VAT Amount</th><th width="10%">NON VAT Amount</th><th width="10%">Zero Tax Amount</th></tr>';
        draw_panel += '</thead><tbody>';
        return draw_panel;
    }

    function close_main_table(vol, total, net, vat, non_vat, zero_vat) {
    ...
    function total_table(item, vol, total, net, vat, non_vat, zero_vat) {
    ...
    function draw_fuel_center(fuel) {
        var draw_panel = '<tr font-size:6px><td width="20%">' + fuel.name + '</td>';
